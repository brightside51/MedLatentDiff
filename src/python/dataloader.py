...
class NCDataset(Dataset):
    ...
    def __getitem__(self, idx: int = 0 or str, save: bool = False):
        ...
        print(img_data.unsqueeze(0)[0, 15:17, :, :].shape)
        return img_data.unsqueeze(0)[0, 15:17, :, :]
   
# Non-Conditional Super-Resolution MetaBrest Dataset Reader Class (V4)
class SRDataset(Dataset):

    # Constructor / Initialization Function
    def __init__(
        self,
        settings1: argparse.ArgumentParser,
        settings2: argparse.ArgumentParser,
        mode: str = 'train'
    ):
        
        # Dataset Choice
        super(SRDataset).__init__()
        self.settings1 = settings1; self.settings2 = settings2

        private_dataset_lr = NCDataset( self.settings1,
                                        mode = mode,
                                        dataset = 'private')
        public_dataset_lr = NCDataset(  self.settings1,
                                        mode = mode,
                                        dataset = 'public')
        self.dataset_lr = ConcatDataset([private_dataset_lr, public_dataset_lr])
        private_dataset_hr = NCDataset( self.settings2,
                                        mode = mode,
                                       dataset = 'private')
        public_dataset_hr = NCDataset(  self.settings2,
                                        mode = mode,
                                        dataset = 'public')
        self.dataset_hr = ConcatDataset([private_dataset_hr, public_dataset_hr])

    # -----------------------------------------------------------------------------------------

    # DataLoader Length / No. Subjects Computation Functionality
    def __len__(self): print(len(self.dataset_lr)); return len(self.dataset_lr)

    # Single Batch / Subject Generation Functionality
    def __getitem__(self, idx: int = 0 or str, save: bool = False):
        
        #print(self.dataset_lr.__getitem__(idx).float().shape)
        #print(self.dataset_hr.__getitem__(idx).float().shape)
        return self.dataset_lr.__getitem__(idx).float(), self.dataset_hr.__getitem__(idx).float() 
